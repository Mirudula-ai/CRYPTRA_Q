...
db = DatabaseManager()
default_relay = os.getenv("RELAY_URL", "http://127.0.0.1:8000").rstrip("/")
if "relay_url_config" not in st.session_state:
    st.session_state.relay_url_config = default_relay

RELAY_URL = st.session_state.relay_url_config

# ---------- RELAY HEALTH CHECK ----------
def check_relay_health():
    try:
        res = requests.get(f"{RELAY_URL}/health", timeout=2)
...
if relay_online:
    ...
    st.sidebar.caption(f"Relay Node unreachable at {RELAY_URL}")

custom_relay = st.sidebar.text_input("Relay Server URL", value=RELAY_URL, key="relay_input")
if custom_relay.rstrip("/") != st.session_state.relay_url_config:
    st.session_state.relay_url_config = custom_relay.rstrip("/")
    st.rerun()
